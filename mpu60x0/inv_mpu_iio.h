...
#include "invensense_mpu60x0.h"

/**
 *  struct inv_mpu60x0_reg_map - Notable registers.
...
/**
 *  struct inv_mpu60x0_chip_config - Cached chip configuration data.
 ...
 */
struct inv_mpu60x0_chip_config {
	unsigned int fsr:2;
	unsigned int lpf:3;
...
/**
 *  struct inv_mpu60x0_hw - Other important hardware information.
 *  @num_reg:	Number of registers on device.
 *  @name:      name of the chip.
 *  @reg:   register map of the chip.
 *  @config:    configuration of the chip.
 */
struct inv_mpu60x0_hw {
	u8 num_reg;
	u8 *name;
	const struct inv_mpu60x0_reg_map *reg;
	const struct inv_mpu60x0_chip_config *config;
};

/*
 *  struct inv_mpu60x0_state - Driver state variables.
 ...
 */
struct inv_mpu60x0_state {
#define TIMESTAMP_FIFO_SIZE 16
	struct iio_trigger  *trig;
	struct inv_mpu60x0_chip_config chip_config;
	const struct inv_mpu60x0_reg_map *reg;
	const struct inv_mpu60x0_hw *hw;
	enum   inv_devices chip_type;
	spinlock_t time_stamp_lock;
	struct i2c_client *client;
	struct spi_device *spi_dev;
	struct inv_mpu60x0_platform_data plat_data;
...
#define INV_MPU60x0_WHO_AM_I					0x75
...
int inv_mpu60x0_write_reg(struct inv_mpu60x0_state *st, int reg, u8 val);
int inv_mpu60x0_read_reg(struct inv_mpu60x0_state *st, int reg, u8 *val);
